...
template<int D> struct attn_bwd_combined_globals { 
    ...
    size_t dynamic_shared_memory() { return MAX_SHARED_MEMORY-8000; }
};
...
__global__ void attend_bwd_combined_ker(const attn_bwd_combined_globals<D> g) {
    ...
    st_bf<BLOCK_SIZE, ATTN_D, ducks::st_layout::row> (&qi_smem)[NUM_WARPS_BWD] = al.allocate<st_bf<BLOCK_SIZE, ATTN_D, ducks::st_layout::row>, NUM_WARPS_BWD>();
    st_bf<BLOCK_SIZE, ATTN_D, ducks::st_layout::row> (&ki_smem)[NUM_WARPS_BWD] = al.allocate<st_bf<BLOCK_SIZE, ATTN_D, ducks::st_layout::row>, NUM_WARPS_BWD>();
    st_bf<BLOCK_SIZE, ATTN_D, ducks::st_layout::row> (&vi_smem)[NUM_WARPS_BWD] = al.allocate<st_bf<BLOCK_SIZE, ATTN_D, ducks::st_layout::row>, NUM_WARPS_BWD>();
    ...
    qkvo_tile<D, bf16, col_l> qkvo_bf16_col;
    qkvo_tile<D, bf16, row_l> qkvo_bf16_row;
    ...
    attn_tile<D, float, row_l> attn_float_row;
    attn_tile<D, bf16, col_l> attn_bf16_col;
    attn_tile<D, bf16, accum_col_l> attn_bf16_accum_col;  
    ...
    using T = typename st_bf<BLOCK_SIZE, ATTN_D>::dtype;
    constexpr int my_bytes_per_memcpy = bytes_per_thread * WARP_THREADS;
    constexpr int my_memcpy_per_tile = BLOCK_SIZE * ATTN_D * sizeof(T) / my_bytes_per_memcpy;
    uint32_t my_swizzled_offsets_Q[my_memcpy_per_tile];
    uint32_t my_swizzled_offsets_V[my_memcpy_per_tile];
    uint32_t my_swizzled_offsets_K[my_memcpy_per_tile];
    prefill_swizzled_offsets<2, false>(qi_smem[warpid()], g.Q, my_swizzled_offsets_Q);
    prefill_swizzled_offsets<2, false>(ki_smem[warpid()], g.K, my_swizzled_offsets_K);
    prefill_swizzled_offsets<2, false>(vi_smem[warpid()], g.V, my_swizzled_offsets_V);
    __builtin_amdgcn_s_waitcnt(0);
    __builtin_amdgcn_sched_barrier(0);
    __builtin_amdgcn_s_barrier();
    load<2, false>(qi_smem[warpid()], g.Q, {b,h,i,0}, my_swizzled_offsets_Q);
    load<2, false>(ki_smem[warpid()], g.K, {b,h,i,0}, my_swizzled_offsets_K);
    load<2, false>(vi_smem[warpid()], g.V, {b,h,i,0}, my_swizzled_offsets_V);
    __builtin_amdgcn_s_waitcnt(0);
    __builtin_amdgcn_sched_barrier(0);
    __builtin_amdgcn_s_barrier();
    __syncthreads();
    load(q_reg,  qi_smem[warpid()]);
    load(k_reg,  ki_smem[warpid()]);
    load(v_reg,  vi_smem[warpid()]);
    ...
    for (int j = 0; j < num_blocks-1; ++j, tic^=1, toc^=1) {
        ...
        swap_layout(qkvo_bf16_col, kj_reg);
        __builtin_amdgcn_s_waitcnt(0);
        __builtin_amdgcn_s_barrier();
        ...
        zero(dOVt);
        copy(qkvo_bf16_row, dOi_reg);
        mma_ABt(dOVt, qkvo_bf16_row, vj_reg, dOVt);
        sub_row(dOVt, dOVt, deltai_vec);
        mul(dOVt, dOVt, S);

        // dQ_i += dS_ij K_j * scale
        swap_layout(attn_float_row, dOVt);
        mul(attn_float_row, attn_float_row, scale_factor);
        attn_tile<D,bf16,row_l> dOVt_bf16_row;
        copy(dOVt_bf16_row, attn_float_row);
        mma_AB(dQ_acc, dOVt_bf16_row, qkvo_bf16_col, dQ_acc);
        ...
        // dV_i += P_ji^T dO_j
        copy(attn_bf16_accum_col, S);
        swap_layout(attn_bf16_col, attn_bf16_accum_col);
        
        copy(qkvo_bf16_row, dOj_reg);
        swap_layout(qkvo_bf16_col, qkvo_bf16_row);
        mma_AtB(dV_acc, attn_bf16_col, qkvo_bf16_col, dV_acc); 
        
        // dS_ji = P_ji ⊙ (dO_j V_i^T − Delta_j) 
        zero(dOVt);
        mma_ABt(dOVt, qkvo_bf16_row, v_reg, dOVt); 
        sub_row(dOVt, dOVt, deltaj_vec);
        mul(dOVt, dOVt, S);
        
        // dK_i += dS_ji^T Q_j * scale
        mul(dOVt, dOVt, scale_factor);
        copy(attn_bf16_accum_col, dOVt);
        swap_layout(attn_bf16_col, attn_bf16_accum_col);
        swap_layout(qkvo_bf16_col, qj_reg);
        mma_AtB(dK_acc, attn_bf16_col, qkvo_bf16_col, dK_acc);
    }
